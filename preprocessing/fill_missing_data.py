...
PATH = "C:/xampp/htdocs/file-storage-local/Data/" + MONTH_NAME + "/" + DAY_NUMBER
...
def check_time_skip(start_dateS, end_dateS, myDates):
    ...
    completeDates = [start_date + timedelta(seconds=x) for x in range(0,(end_date-start_date ).seconds + 1)]
    completeDates = [d.strftime("%H:%M:%S") for d in completeDates] # Convert date to string
...
def fix_missing_dates(df, missing_dates, files):
    ...
    for file, i in zip(files, spans):
        data = df[i:i+300].to_csv(header=False, index=False, line_terminator="\n")
