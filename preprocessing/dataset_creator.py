...
CLIENT_ID='22BYJY'
CLIENT_SECRET='7bb185bc59cd98549ea59df136a4621a'

path = "C:/xampp/htdocs/file-storage-local/Data/"

SAVE_PATH = "D:/FitBiT/python_api/Fitbit_API/datasets"
class DatasetCreator():
    def __init__(self, dates=None, path=None):
        ...
        self.predefined_skip_features = ['hrm_p10', 'accel_var', 'hrm_med', 'hrm_p90']
    ...
    def get_sleep_data(self, date):
        ...
        df["level"], self.labels = pd.factorize(df["level"]) # changes labels ["wake", "light", "deep", "rem"] into [0, 1, 2, 3]
        # df["level"], self.labels = pd.factorize(df["level"])[0]
        return df
    ...
    def generate_labaled_dataset(self, date):
        ...
        dates.append(self.get_last_time_stage(dates[-1], sleep_df["seconds"][sleep_df.index[-1]])) # adds last 
        dts = [dt.strftime('%Y-%m-%d %H:%M:%S') for dt in self.datetime_range(dates[0], dates[-1], timedelta(seconds=1))]
        ...
        for idx, d in enumerate(dates):
            short_dates.append([d, d + timedelta(seconds=int( short_data["seconds"][short_data.index[idx]]))])
        new_df = []
        for idx, d in enumerate(short_dates):
            dts = [dt.strftime('%Y-%m-%d %H:%M:%S') for dt in self.datetime_range(d[0], d[-1], timedelta(seconds=1))]
            labels = self.make_labels_short_data(short_data["seconds"][short_data.index[idx]], short_data["level"][short_data.index[idx]])
        ...
        completeName = os.path.join(SAVE_PATH, "testttttt.csv")
        f = open(completeName, "w")
        f.write(csv_temp)
        f.close()

        dataset = self.get_one_day(month=date.strftime("%B"), day=str(date.day))
    ...
    def cut_data_to_chunks(self):
        ...
        while i < self.processed_data.shape[0]-self.chunk_size:
            start = datetime.strptime(self.processed_data["time"][i], "%Y-%m-%d %H:%M:%S")
            modulo = start.second % self.chunk_size
            if modulo == 0:
                end = datetime.strptime(self.processed_data["time"][i+self.chunk_size], "%Y-%m-%d %H:%M:%S")
                if end - start == span:
                    f, l = self.features_labels(self.processed_data[i:i+self.chunk_size])
    ...
    def change_data_to_chunks(self):
        print("Chunking...")
        self.processed_data = pd.DataFrame({"time": self.raw_data["time"], "hrm" : self.raw_data["hrm"], "label": self.raw_data["label"]})
        ...
        title = ['hrm_mean', 'hrm_min', 'hrm_max', 'hrm_med', 'hrm_var', 'hrm_kurtosis', 'hrm_skew', 'hrm_p10', 'hrm_p90', 'accel_mean', 'accel_min', 'accel_max', 'accel_med', 'accel_var', 'accel_kurtosis', 'accel_skew', 'accel_p10', 'accel_p90']
    ...
    def process_accelerometer_data(self):
        ...
        accel_sum = np.sqrt(np.array(accel_x)**2 +np.array(accel_y)**2 + np.array(accel_z)**2)
        self.processed_data["accel"] = accel_sum

    def remove_unnecessary_features(self, auto=False):
        if auto:
            self.processed_data = self.processed_data.drop(columns=self.predefined_skip_features)
        else:
            fs = FeatureSelector(data = self.processed_data.drop("label", axis=1), labels = self.processed_data["label"])
